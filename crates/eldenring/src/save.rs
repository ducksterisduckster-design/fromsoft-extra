...
use ilhook::{x64::*, *};
use pelite::pe64::Pe;
use shared::{FromStatic, Program, ext::*};
...
/// Registers callbacks to add data to the Elden Ring save file.
///
/// ## Callbacks
///
/// The `on_save` callback is called whenever Elden Ring saves (other than for
/// the partial save file used to save main menu settings). The callback returns
/// a byte vector which provides arbitrary data to write to the save file, with
...
///
/// The `on_load` callback is called whenever Elden Ring loads a save file
/// (*including* the partial file used to save main menu settings). It takes an
/// [OnLoadType] which provides information about the file, including the data
/// saved by `on_save` if it was provided for the current file.
///
/// ## Safety
///
    ...
    OnSave: (Fn() -> Option<Vec<u8>>) + Send + Sync + 'a,
{
    let callback = move |reg: *mut Registers, original| {
        ...
        // Safety: We trust that Elden Ring gives us valid pointers.
    ...
    OnLoad: Fn(OnLoadType) + Send + Sync + 'a,
{
    let callback = move |reg: *mut Registers, original| {
        ...
        let version = unsafe { (*reg).r8 as u32 };
        ...
        if original(this, stream, version) == 0 {
            return 0;
        }
...
trait EquipGameDataExt {
    fn is_main_menu(&self) -> bool;
}

impl EquipGameDataExt for EquipGameData {
    fn is_main_menu(&self) -> bool {
